				...
				actions : {
										...
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("PurchaseRequisitionKey") === oObject.PurchaseRequisitionKey;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].PurchaseRequisitionKey;
										} else {
											sCurrentId = aItemsNotInTheList[0].PurchaseRequisitionKey;
										}
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("PurchaseRequisitionKey"),
							title: oBindingContext.getProperty("ProductDescription")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "PurchaseRequisitions (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'PurchaseRequisitions (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("ProductDescription");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all PurchaseRequisitions to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "PurchaseRequisitions (" + iExpectedLength + ")"}),
									success : function () {
