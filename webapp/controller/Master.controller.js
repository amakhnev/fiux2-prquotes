...
sap.ui.define([
		...
		"edu/opensap/fiux2/am/model/grouper",
		...
		return BaseController.extend("edu.opensap.fiux2.am.controller.Master", {
			...
			onInit : function () {
				...
				var fnOnApprovalExecuted = function(channel, event, data) {
					this.onApprovalExecuted(channel, event, data);
				},
				oEventBus = this.getEventBus();
				
				// Subscribe to event that is triggered by other screens when a PR was approved or rejected
				oEventBus.subscribe("edu.opensap.fiux2.am", "approvalExecuted", fnOnApprovalExecuted.bind(this));
			},
			...
			onSearch : function (oEvent) {
				...
				if (sQuery) {
					this._oListFilterState.aSearch = [new Filter("ProductDescription", FilterOperator.Contains, sQuery)];
				} else {
			...
			onOpenViewSettings : function () {
				if (!this._oViewSettingsDialog) {
					this._oViewSettingsDialog = sap.ui.xmlfragment("edu.opensap.fiux2.am.view.ViewSettingsDialog", this);
			...
			onConfirmViewSettingsDialog : function (oEvent) {
				...
				aFilterItems.forEach(function (oItem) {
					switch (oItem.getKey()) {
						case "Filter1" :
							aFilters.push(new Filter("DeliveryDateString", FilterOperator.LE, formatter.dateForCompare(today30)));
							break;
						case "Filter2" :
							aFilters.push(new Filter("DeliveryDateString", FilterOperator.BT, formatter.dateForCompare(today30),formatter.dateForCompare(today60)));
							break;
						case "Filter3" :
							aFilters.push(new Filter("DeliveryDateString", FilterOperator.GE, formatter.dateForCompare(today60)));
							break;
						default :
							break;
					}
			...
			onApprovalExecuted: function(channel, event, data) {
				var sKey = data.prKey;
				// get next item 
				this.onRefresh();

				var aItems = this._oList.getItems(), oItemToSelect; 
		
				
//				oItemToSelect	= aItems[0]; // Display the first PR in the list
				
				for (var i=0;i< aItems.length;i++){
					if (aItems[i].getBindingContext().getProperty("PurchaseRequisitionKey") === sKey) {
						oItemToSelect = aItems[i === aItems.length - 1 ? (i - 1) : (i + 1)];
						break;
					}
				}
			...
			_onMasterMatched :  function() {
				this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
					function (mParams) {
						...
						var sObjectId = mParams.firstListitem.getBindingContext().getProperty("PurchaseRequisitionKey");
			...
			_showDetail : function (oItem) {
				...
				this.getRouter().navTo("object", {
					objectId : oItem.getBindingContext().getProperty("PurchaseRequisitionKey")
				}, bReplace);

			},

			_showNoObjects : function () {
				this.getRouter().getTargets().display("detailNoObjectsAvailable");					
			},
